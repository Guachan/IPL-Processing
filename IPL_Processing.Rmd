...
## How to extract linear regression formulas to apply calibrations?
...
data2 <- data.frame(Total_concentrations2) %>% 
  filter(Conc.L >= 0)

t3_ipls <- data2 %>% filter(Station == 'T3')


t3_ipl_plot <- aggregate(Conc.L ~ Class + Depth, data = t3_ipls, sum)

total_t3 <- aggregate(Conc.L ~ Depth, data = t3_ipl_plot, sum) %>% 
  mutate(Total = Conc.L) %>% select(Depth, Total)



t3_ipl_plot <- left_join(by = 'Depth', t3_ipl_plot, total_t3) %>% 
  mutate(Percentage = Conc.L/Total*100) %>% 
  arrange(desc(Class))

##t3_ipl_plot <- t3_ipl_plot[-c(11, 17, 18),]
t3_ipl_plot <- t3_ipl_plot %>% 
  mutate(order = c(1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5))


t5_ipls <- data2 %>% filter(Station == 'T5')


t5_ipl_plot <- aggregate(Conc.L ~ Class + Depth, data = t5_ipls, sum)

total_t5 <- aggregate(Conc.L ~ Depth, data = t5_ipl_plot, sum) %>% 
  mutate(Total = Conc.L) %>% select(Depth, Total)

t5_ipl_plot <- left_join(by = 'Depth', t5_ipl_plot, total_t5) %>% 
  mutate(Percentage = Conc.L/Total*100) %>% 
  arrange(desc(Class))

##t5_ipl_plot <- t5_ipl_plot[-c(16, 17, 18),]

t5_ipl_plot <- t5_ipl_plot %>% 
  mutate(order = c(1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5, 1:1:5))
```
