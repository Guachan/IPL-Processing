...
read_plus <- function(flnm) {
    read_csv(flnm, col_names = TRUE) %>% ## For some reason these csvs do weird things when trying to skip rows in the import
        mutate(filename = flnm) ### Add filename to read_csv import
}
setwd("C:/Users/canta/IPL-Processing/Data")
samples <-
    list.files(pattern = "*.csv", 
               full.names = T) %>% 
    map_df(~read_plus(.)) %>% 
    select(c("Compound", "Response", "filename")) %>% 
    ...
    na.omit() %>% 
    unique() %>% 
  mutate(OG = as.numeric(substr(filename, 5, 6)), ## relevant infomration from sample names (depth, station etc)
         station = substr(filename, 8, 9),
         depth = as.numeric(substr(filename, 11, 13)),
         fraction = 0.3)
```
...
samples <- merge(samples, IS_Values, by = "OG") %>% 
