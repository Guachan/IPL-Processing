...
## Sample Data Import
```{r}
knitr::include_graphics("Kosmos_Maps_Draft_Small.png")
```
...
samples <- samples %>% unnest(.id = 'filename') %>% 
 ...
 # na.omit() %>% # might want to keep nas to leave in "0" concentrations for some IPL classes
  unique() %>%
  mutate(
    OG = str_match(filename, pattern) %>% { .[,2] }, #Cleaning up variable names
...
### Best to use calibration standards from the same run as your samples (or as near as possible)

```{r, warning=FALSE, message=FALSE}
path <- here("Data", "Compound Calibration Report_IPLS standards.xlsx")
stopifnot(file.exists(path))
# pull out 16 calibrations from single xls file
calibs <- 
  data_frame(
    sheet_index = 1:16,
...
```{r, message=FALSE}
## Assign intercept and slope variables for each standard
linear_model_fit <- calib_fits %>% unnest(lm_coefs)
MGDG_int <- linear_model_fit$estimate[3]
MGDG_slope <- linear_model_fit$estimate[4]
GLC_Cer_int <- linear_model_fit$estimate[5]
GLC_Cer_slope <- linear_model_fit$estimate[6]
...
PC_int <- linear_model_fit$estimate[27]
PC_slope <- linear_model_fit$estimate[28] 
...
## couldn't figure out how the crossing method Seb suggested would work, might need to clean up the calibration sheet names first. This is clunky, but serves as a place holder for now
...
#                      grepl("C16-PC", Compound) ~ (Response - PC_int)/PC_slope,
...
#                      grepl("DGTA", Compound) ~ (Response - DGTS_int)/DGTS_slope,
...
#                      grepl("1-GLC-CER", Compound) ~ (Response - GLC_Cer_int)/GLC_Cer_slope,
...
samples_calib <- samples %>% ## This works but clunky
  arrange(Compound) %>% 
  mutate(calibrated = 
           case_when(grepl("MGDG", Compound) ~ Response/GLC_Cer_slope, ##apply calib based on str
                     grepl("PE-DAG/AEG-\\d+", Compound) ~ Response/PE_slope,
                     grepl("PE-DEG-\\d+", Compound) ~ Response/PE_slope,
                     grepl("PG", Compound) ~ Response/PG_slope,
                     grepl("PC-\\d+", Compound) ~ Response/PC_slope,
                     grepl("C16-PC", Compound) ~ Response/PC_slope,
                     grepl("PC-DEG-\\d+", Compound) ~ Response/PC_slope,
                     grepl("PC-A", Compound) ~ Response/PC_AR_slope,
                     grepl("PDME", Compound) ~ Response/PDME_slope,
                     grepl("PME", Compound) ~ Response/PME_slope,
                     grepl("DGTS", Compound) ~ Response/DGTS_slope,
                     grepl("DGTA", Compound) ~ Response/DGTS_slope,
                     grepl("SQ", Compound) ~ Response/SQDAG_slope,
                     grepl("PE-A", Compound) ~ Response/PE_AR_slope,
                     grepl("DGDG", Compound) ~ Response/DGDG_slope,
                     grepl("C16PAF", Compound) ~ Response/C16_PAF_slope,
                     #grepl("1-GLC-CER", Compound) ~ Response/GLC_Cer_slope,
                     grepl("DGCC", Compound) ~ Response/SQDAG_slope)) %>% 
         mutate(calibrated = if_else(calibrated > 0, calibrated, 0)) ## Keep 0 values for simplicity in plotting later
...
Lowphox2_notes <- filter(
  ...
  mutate(Fraction_of_total = Total_Injected2 / (TLE_mg*1000), # Calculate fraction injected out of total
...
IS_Measured <- samples_calib %>% 
  ...
  rename(calibrated_IS = calibrated)## Code block for extracting internal standard from fullscan run
  

samples_IS <- merge(samples_calib, IS_Measured, by = "OG") %>% 
  merge(Lowphox2_notes, by = "OG") ## Merge every sample with the internal standard measured for that sample

IS_corrected_samples <- samples_IS %>% ## Apply internal standard calibration only if IS is lower in sample than what was added
    mutate(
      iscorrected = if_else(
...
### Need to add class column to df for comparing samples by IPL classes, not structure
```{r, message=FALSE}
final_corrected <- final_corrected %>% 
  mutate(Class =
           case_when(grepl("MGDG", Compound) ~ "MG-DAG",
                     ...
                     grepl("DGTA", Compound) ~ "DGTS+DGTA",
                     grepl("SQ", Compound) ~ "SQ-DAG/AEG",
                     grepl("PE-A", Compound) ~ "P-Archaeol",
                     grepl("DGDG", Compound) ~ "DG-DAG",
                     #grepl("1-GLC-CER", Compound) ~ "Gly-Cer",
                     grepl("DGCC", Compound) ~ "DGCC")) %>% 
  na.omit()
```

## Calculate some useful indices, totals for each class and depth etc.
...
t3_ipl_plot <- left_join(by = c('depth', 'fraction'), total_yield_t3, t3_ipls_2) %>% ## Join totals for each depth with all molecule measurments
  mutate(percentage = total_L/Total *100) %>% 
  arrange(desc(Class))

t3_ipl_plot_0.3 <- t3_ipl_plot %>% 
  filter(fraction == '0.3') %>% 
  mutate(order = c(1:1:6, 1:1:6, 1:1:6, 1:1:6, 1:1:6, 1:1:6, 1:1:6, 1:1:6, 1:1:6, 1:1:6, 1:1:6, 1:1:6, 1:1:6)) #Weird relic of processing method not indicating all 0 values
...
### Station 5
```{r, message=FALSE}
t5_ipls_1 <- final_corrected %>% filter(station == '5')## Separate stations
t5_ipls_2 <- aggregate(total_L ~ Class + depth + fraction, data = t5_ipls_1, sum) ## totals for each class at each depth and fraction

total_yield_t5 <- aggregate(total_L ~ depth + fraction, data = t5_ipls_2, sum) %>% # totals for each depth by fraction
  mutate(Total = total_L) %>% 
  select(depth, fraction, Total)
...
## Plot bar graphs of relative concentration

### Station 3, 0.3 vs 2.7 Fraction
